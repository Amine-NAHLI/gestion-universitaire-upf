...
@section('title', __('Gestion') . ' — ' . $module->nom)
...
<div x-data="{ activeTab: 'annonces' }" class="space-y-6">
        ...
        <div class="lg:col-span-1">
            <div class="bg-white dark:bg-gray-800 rounded-3xl border border-gray-100 dark:border-gray-700 shadow-xl overflow-hidden sticky top-8">
                <div class="p-6 border-b border-gray-50 dark:border-gray-700 bg-gray-50 dark:bg-gray-900/50">
                    <h4 class="text-[10px] font-black text-gray-800 dark:text-white uppercase tracking-widest">{{ __('Publier une annonce') }}</h4>
                </div>
                <form action="{{ route('professeur.classroom.annonce', $module) }}" method="POST" class="p-6 space-y-4">
                    @csrf
                    <div class="space-y-1">
                        <label class="text-[10px] font-black text-gray-400 uppercase tracking-widest">{{ __('Titre') }}</label>
                    ...
                    <div class="space-y-1">
                        <label class="text-[10px] font-black text-gray-400 uppercase tracking-widest">{{ __('Contenu') }}</label>
                    ...
                    <div class="flex items-center gap-3 p-3 bg-gray-50 dark:bg-gray-900/50 rounded-xl">
                        ...
                        <span class="text-xs font-bold text-gray-600 dark:text-gray-400">{{ __('Épingler en haut du flux') }}</span>
                    </div>
            ...
            <div class="bg-white dark:bg-gray-800 rounded-3xl border border-gray-100 dark:border-gray-700 shadow-xl overflow-hidden {{ $annonce->epinglee ? 'ring-2 ring-indigo-500' : '' }}">
                <div class="p-8">
                    <div class="flex justify-between items-start mb-4">
                        ...
                        @if($annonce->epinglee)
                            <span class="bg-indigo-100 text-indigo-600 px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest">{{ __('Épinglée') }}</span>
                        @endif
                    </div>
                    ...
                    <div class="flex items-center justify-between pt-6 border-t border-gray-50 dark:border-gray-700">
                        <span class="text-[10px] text-gray-400 font-bold uppercase tracking-widest">{{ __('Posté') }} {{ $annonce->created_at->locale(app()->getLocale())->diffForHumans() }}</span>
                        <span class="text-[10px] font-black text-indigo-500 uppercase tracking-widest">{{ $annonce->commentaires->count() }} {{ __('Commentaires') }}</span>
                    </div>
                </div>
            </div>
            @endforeach
    ...
    <div x-show="activeTab === 'supports'" class="grid grid-cols-1 lg:grid-cols-3 gap-8" x-transition>
        ...
        <div class="lg:col-span-1">
            <div class="bg-white dark:bg-gray-800 rounded-3xl border border-gray-100 dark:border-gray-700 shadow-xl overflow-hidden sticky top-8">
                <div class="p-6 border-b border-gray-50 dark:border-gray-700 bg-gray-50 dark:bg-gray-900/50">
                    <h4 class="text-[10px] font-black text-gray-800 dark:text-white uppercase tracking-widest">{{ __('Déposer un support') }}</h4>
                </div>
                <form action="{{ route('professeur.classroom.support', $module) }}" method="POST" enctype="multipart/form-data" class="p-6 space-y-4">
                    @csrf
                    <div class="space-y-1">
                        <label class="text-[10px] font-black text-gray-400 uppercase tracking-widest">{{ __('Titre du document') }}</label>
                        <input type="text" name="titre" required class="w-full rounded-xl border-gray-200 dark:border-gray-700 dark:bg-gray-900 dark:text-white text-sm focus:ring-indigo-500">
                    </div>
                    <div class="space-y-1">
                        <label class="text-[10px] font-black text-gray-400 uppercase tracking-widest">{{ __('Description (optionnel)') }}</label>
                    ...
                    <div class="space-y-1">
                        <label class="text-[10px] font-black text-gray-400 uppercase tracking-widest">{{ __('Fichier (PDF, PPT, DOC, ZIP...)') }}</label>
                    ...
                    <table class="w-full text-left border-collapse">
                        <thead>
                            <tr class="bg-gray-50 dark:bg-gray-900/50">
                                ...
                                <th class="px-6 py-4 text-[10px] font-black text-gray-400 uppercase tracking-widest">{{ __('Taille') }}</th>
                        ...
                        <tbody class="divide-y divide-gray-100 dark:divide-gray-700">
                            ...
                            <tr class="hover:bg-gray-50/50 dark:hover:bg-gray-700/30 transition-colors">
                                <td class="px-6 py-4">
                                    ...
                                    <span class="text-[10px] text-gray-400 font-bold uppercase">{{ __('Ajouté le') }} {{ $sup->created_at->format('d/m/Y') }}</span>
                                </td>
                                <td class="px-6 py-4">
                                ...
                                <td class="px-6 py-4 text-center">
                                    <a href="{{ asset('storage/' . $sup->fichier) }}" target="_blank" class="text-indigo-600 hover:text-indigo-700 font-black text-[10px] uppercase tracking-widest">{{ __('Consulter') }}</a>
                                </td>
                            </tr>
                            @endforeach
                        </tbody>
