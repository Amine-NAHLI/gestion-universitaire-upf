...
<div class="space-y-8" x-data="{ type: 'attestation_travail' }">
    ...
    <div class="bg-indigo-600 rounded-3xl p-8 text-white shadow-2xl relative overflow-hidden">
        <div class="relative z-10">
            ...
            <p class="text-indigo-100 text-sm mb-8">Sélectionnez le type de document dont vous avez besoin.</p>
            
            <form action="{{ route('professeur.demandes.store') }}" method="POST" class="space-y-6 max-w-2xl">
                @csrf
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div class="space-y-1">
                        <label class="text-[10px] font-black text-indigo-200 uppercase tracking-widest">Type de document</label>
                        <select name="type" x-model="type" required class="w-full rounded-xl bg-white/10 border-white/20 text-white text-sm focus:ring-white">
                            <option value="attestation_travail" class="text-gray-900">Attestation de travail</option>
                            <option value="ordre_mission" class="text-gray-900">Ordre de mission</option>
                        </select>
                    </div>
                </div>
                ...
                <div x-show="type === 'ordre_mission'" x-transition class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 p-6 bg-white/5 rounded-2xl border border-white/10">
                    <div class="space-y-1">
                        <label class="text-[10px] font-black text-indigo-200 uppercase tracking-widest">Destination</label>
                    ...
                    <div class="space-y-1">
                        <label class="text-[10px] font-black text-indigo-200 uppercase tracking-widest">Date Départ</label>
                    ...
                    <div class="space-y-1">
                        <label class="text-[10px] font-black text-indigo-200 uppercase tracking-widest">Date Retour</label>
                    ...
                    <div class="space-y-1">
                        <label class="text-[10px] font-black text-indigo-200 uppercase tracking-widest">Motif Mission</label>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-3xl border border-gray-100 dark:border-gray-700 shadow-xl overflow-hidden">
        <div class="p-6 border-b border-gray-100 dark:border-gray-700">
            <h3 class="text-lg font-black text-gray-800 dark:text-white uppercase tracking-wider">Historique des demandes</h3>
        </div>
            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="bg-gray-50 dark:bg-gray-900/50">
                        ...
                        <th class="px-6 py-4 text-[10px] font-black text-gray-400 uppercase tracking-widest">Date Demande</th>
                    ...
                    <tr class="hover:bg-gray-50/50 dark:hover:bg-gray-700/30 transition-colors">
                        <td class="px-6 py-4">
                            <span class="text-sm font-bold text-gray-800 dark:text-white">
                                {{ $demande->type === 'attestation_travail' ? 'Attestation de travail' : 'Ordre de mission' }}
                            </span>
                        </td>
                        <td class="px-6 py-4">
                        ...
                        <td class="px-6 py-4 text-center">
                            @if($demande->statut === 'validee' && $demande->fichier_pdf)
                                <a href="{{ route('etudiant.demandes.download', $demande) }}" class="text-indigo-600 hover:text-indigo-700 font-black text-[10px] uppercase tracking-widest">Télécharger PDF</a>
                            @else
