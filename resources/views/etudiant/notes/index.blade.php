...
@section('page-title', __('Mes Notes'))
...
<div class="space-y-6">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div class="bg-white dark:bg-gray-800 p-6 rounded-3xl border border-gray-100 dark:border-gray-700 shadow-xl">
            <p class="text-xs font-black text-gray-400 uppercase tracking-widest mb-2">{{ __('Moyenne Générale') }}</p>
        ...
        <div class="bg-white dark:bg-gray-800 p-6 rounded-3xl border border-gray-100 dark:border-gray-700 shadow-xl">
            <p class="text-xs font-black text-gray-400 uppercase tracking-widest mb-2">{{ __('Modules Validés') }}</p>
            <div class="flex items-baseline gap-2">
                <span class="text-4xl font-black text-indigo-500">{{ $modules_valides }}</span>
                <span class="text-sm font-bold text-gray-400">/ {{ $modules_total }} {{ __('Modules') }}</span>
            </div>
        ...
        <div class="bg-white dark:bg-gray-800 p-6 rounded-3xl border border-gray-100 dark:border-gray-700 shadow-xl flex flex-col justify-center">
            <p class="text-xs font-black text-gray-400 uppercase tracking-widest mb-1">{{ __('Année Universitaire') }}</p>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-3xl border border-gray-100 dark:border-gray-700 shadow-xl overflow-hidden">
        <div class="p-6 border-b border-gray-100 dark:border-gray-700 flex justify-between items-center">
            <h3 class="text-lg font-black text-gray-800 dark:text-white uppercase tracking-wider">{{ __('Relevé de Notes Détaillé') }}</h3>
        </div>
            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="bg-gray-50 dark:bg-gray-900/50">
                        ...
                        <th class="px-6 py-4 text-[10px] font-black text-gray-400 uppercase tracking-widest text-center">{{ __('CC1') }}</th>
                        <th class="px-6 py-4 text-[10px] font-black text-gray-400 uppercase tracking-widest text-center">{{ __('CC2') }}</th>
                        <th class="px-6 py-4 text-[10px] font-black text-gray-400 uppercase tracking-widest text-center">{{ __('Examen') }}</th>
                ...
                <tbody class="divide-y divide-gray-100 dark:divide-gray-700">
                    ...
                    <tr>
                        <td colspan="6" class="px-6 py-12 text-center text-gray-500 dark:text-gray-400">
                            {{ __('Aucune note disponible pour le moment.') }}
                        </td>
                    </tr>
                    @endforelse
                </tbody>
                <tfoot class="bg-gray-50/50 dark:bg-gray-900/20">
                    <tr>
                        <td colspan="4" class="px-6 py-4 text-right text-xs font-black text-gray-400 uppercase tracking-widest">{{ __('Moyenne du semestre') }}</td>
                        <td class="px-6 py-4 text-center">
