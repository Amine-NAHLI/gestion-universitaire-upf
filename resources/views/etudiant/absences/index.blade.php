...
@section('title', __('Mes Absences'))
@section('page-title', __('Mes Absences'))
...
<div class="space-y-6">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div class="bg-white dark:bg-gray-800 p-6 rounded-3xl border border-gray-100 dark:border-gray-700 shadow-xl">
            <p class="text-xs font-black text-gray-400 uppercase tracking-widest mb-2">{{ __('Total Absences') }}</p>
            <span class="text-4xl font-black text-gray-800 dark:text-white">{{ $stats['total'] }}</span>
        </div>

        <div class="bg-white dark:bg-gray-800 p-6 rounded-3xl border border-gray-100 dark:border-gray-700 shadow-xl">
            <p class="text-xs font-black text-gray-400 uppercase tracking-widest mb-2">{{ __('Justifiées') }}</p>
            <span class="text-4xl font-black text-emerald-500">{{ $stats['justifiees'] }}</span>
        </div>

        <div class="bg-white dark:bg-gray-800 p-6 rounded-3xl border border-gray-100 dark:border-gray-700 shadow-xl">
            <p class="text-xs font-black text-gray-400 uppercase tracking-widest mb-2">{{ __('Non Justifiées') }}</p>
    ...
    <div class="bg-red-50 border-l-4 border-red-500 p-4 rounded-xl flex items-center gap-4 animate-pulse">
        ...
        <div>
            <p class="text-red-800 font-black uppercase text-xs tracking-wider">{{ __('Alerte Exclusion') }}</p>
            <p class="text-red-700 text-sm">{{ __("Attention : vous avez dépassé 3 absences. Risque d'exclusion majeur.") }}</p>
        </div>
    </div>
    @endif
    ...
    <div class="bg-white dark:bg-gray-800 rounded-3xl border border-gray-100 dark:border-gray-700 shadow-xl overflow-hidden">
        <div class="p-6 border-b border-gray-100 dark:border-gray-700">
            <h3 class="text-lg font-black text-gray-800 dark:text-white uppercase tracking-wider">{{ __('Historique des Absences') }}</h3>
        </div>
        <div class="overflow-x-auto">
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="bg-gray-50 dark:bg-gray-900/50">
                        <th class="px-6 py-4 text-[10px] font-black text-gray-400 uppercase tracking-widest">{{ __('Module') }}</th>
                        <th class="px-6 py-4 text-[10px] font-black text-gray-400 uppercase tracking-widest">{{ __('Date & Heure') }}</th>
                        <th class="px-6 py-4 text-[10px] font-black text-gray-400 uppercase tracking-widest text-center">{{ __('Justifiée') }}</th>
                        <th class="px-6 py-4 text-[10px] font-black text-gray-400 uppercase tracking-widest text-center">{{ __('Statut Justificatif') }}</th>
                ...
                <tbody class="divide-y divide-gray-100 dark:divide-gray-700">
                    ...
                    <tr class="hover:bg-gray-50/50 dark:hover:bg-gray-700/30 transition-colors">
                        ...
                        <td class="px-6 py-4 text-center">
                            ...
                            @else
                                <span class="text-xs text-gray-400 italic">{{ __('Aucun document') }}</span>
                            @endif
                        </td>
                        <td class="px-6 py-4 text-center">
                            ...
                            @else
                                <span class="text-[10px] text-gray-400 font-bold uppercase">{{ __('Clôturé') }}</span>
                            @endif
                        </td>
                    </tr>
                    @empty
                    <tr>
                        <td colspan="5" class="px-6 py-12 text-center text-gray-500 dark:text-gray-400 font-bold italic">
                            {{ __("Félicitations ! Vous n'avez aucune absence enregistrée.") }}
                        </td>
                    </tr>
                    @endforelse
                </tbody>
