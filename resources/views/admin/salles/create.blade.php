...
@section('title', __('Gestion des Salles'))
@section('page-title', isset($salle) ? __('Modifier la salle') : __('Nouvelle salle'))
    ...
    <div class="bg-white dark:bg-gray-800 rounded-3xl border border-gray-100 dark:border-gray-700 shadow-xl overflow-hidden">
        <div class="p-8 border-b border-gray-100 dark:border-gray-700 bg-gray-50/50 dark:bg-gray-900/30">
            <h2 class="text-2xl font-extrabold text-gray-800 dark:text-white">
                {{ isset($salle) ? __('Modifier') . ' : ' . $salle->nom : __('Créer une salle') }}
            </h2>
        </div>
        ...
        <form action="{{ isset($salle) ? route('admin.salles.update', $salle) : route('admin.salles.store') }}"
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="space-y-2">
                    <label for="nom" class="text-sm font-bold text-gray-700 dark:text-gray-300">{{ __('Nom / Numéro de salle') }}</label>
                ...
                <div class="space-y-2">
                    <label for="capacite" class="text-sm font-bold text-gray-700 dark:text-gray-300">{{ __('Capacité (places)') }}</label>
                ...
                <div class="space-y-2">
                    <label for="type" class="text-sm font-bold text-gray-700 dark:text-gray-300">{{ __('Type de salle') }}</label>
                    <select name="type" id="type" required
                            class="w-full px-4 py-3 rounded-xl border border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-800 dark:text-white focus:ring-2 focus:ring-indigo-500">
                        <option value="cours" {{ old('type', $salle->type ?? '') == 'cours' ? 'selected' : '' }}>{{ __('Salle de cours') }}</option>
                        <option value="td" {{ old('type', $salle->type ?? '') == 'td' ? 'selected' : '' }}>{{ __('Salle de TD') }}</option>
                        <option value="tp" {{ old('type', $salle->type ?? '') == 'tp' ? 'selected' : '' }}>{{ __('Salle de TP (Labo)') }}</option>
                        <option value="amphi" {{ old('type', $salle->type ?? '') == 'amphi' ? 'selected' : '' }}>{{ __('Amphithéâtre') }}</option>
                    </select>
                </div>
                ...
                <div class="flex items-center h-full pt-8">
                    <label class="relative inline-flex items-center cursor-pointer">
                        ...
                        <span class="ml-3 text-sm font-bold text-gray-700 dark:text-gray-300">{{ __('Salle opérationnelle') }}</span>
                    </label>
                </div>
            </div>

            <div class="space-y-2">
                <label for="equipements" class="text-sm font-bold text-gray-700 dark:text-gray-300">{{ __('Équipements (vidéoprojecteur, PC, clim...)') }}</label>
            ...
            <div class="flex items-center justify-end gap-4 pt-6 border-t border-gray-100 dark:border-gray-700">
                <a href="{{ route('admin.salles.index') }}" class="btn-secondary">{{ __('Annuler') }}</a>
                <button type="submit" class="btn-primary px-8">
                    {{ isset($salle) ? __('Mettre à jour') : __('Créer la salle') }}
                </button>
            </div>
        </form>
    </div>
