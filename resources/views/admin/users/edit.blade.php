...
@section('title', __('Modifier') . ' ' . $user->full_name)
@section('page-title', __('Modifier') . ' : ' . $user->full_name)
...
<div class="max-w-4xl mx-auto">
    ...
    <div class="bg-white dark:bg-gray-800 rounded-3xl border border-gray-100 dark:border-gray-700 shadow-xl overflow-hidden">
        <div class="p-8 border-b border-gray-100 dark:border-gray-700 bg-gray-50/50 dark:bg-gray-900/30 flex items-center gap-4">
            ...
            <div>
                <h2 class="text-2xl font-extrabold text-gray-800 dark:text-white">{{ __('Modifier') }} : {{ $user->full_name }}</h2>
                <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">ID #{{ $user->id }} • {{ $user->created_at->format('d/m/Y') }}</p>
            </div>
        </div>
        ...
        <div x-data="{
                ...
                filiereId: '{{ old('filiere_id', $filiereId) }}',
                niveaux: window._userEditData.niveaux,
                niveauId: '{{ old('niveau_id', $niveauId) }}',
                groupes: window._userEditData.groupes,
                groupeId: '{{ old('groupe_id', $groupeId) }}',
                modulesParFiliere: window._userEditData.modulesParFiliere,
                modulesSelectionnes: window._userEditData.modulesSelectionnes,
                searchModule: '',
            ...
            <form action="{{ route('admin.users.update', $user) }}" method="POST" class="p-8 space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="space-y-2">
                        ...
                        <select name="role" x-model="role" required
                            ...
                            <option value="etudiant" {{ old('role', $user->role) == 'etudiant' ? 'selected' : '' }}>{{ __('Étudiant') }}</option>
                            <option value="professeur" {{ old('role', $user->role) == 'professeur' ? 'selected' : '' }}>{{ __('Professeur') }}</option>
                ...
                <div class="p-6 bg-indigo-50/50 dark:bg-indigo-900/10 rounded-2xl border border-indigo-100 dark:border-indigo-900/30">
                    <div class="flex items-center gap-3 mb-4">
                        ...
                        <h3 class="text-sm font-bold text-indigo-800 dark:text-indigo-300">{{ __('Sécurité (Optionnel)') }}</h3>
                    </div>
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div class="space-y-2">
                            <label class="text-sm font-bold text-gray-700 dark:text-gray-300">{{ __('Nouveau mot de passe') }}</label>
                            <input type="password" name="password" placeholder="{{ __('Laisser vide pour ne pas changer') }}"
                                   class="w-full px-4 py-3 rounded-xl border border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-800 dark:text-white focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition-all">
                    ...
                    <div class="flex items-center gap-2 mb-2">
                        ...
                        @if($user->isEtudiant() && $user->etudiant)
                            <span class="ml-auto text-xs text-gray-400">
                                {{ __('Matricule') }}: {{ $user->etudiant->matricule }} • CNE: {{ $user->etudiant->cne }}
                            </span>
                        @endif
                    </div>
                        ...
                        <div class="space-y-2">
                            ...
                            <select name="filiere_id" x-model="filiereId" @change="onFiliereChange()"
                                ...
                                @foreach($filieres as $filiere)
                                    <option value="{{ $filiere->id }}" {{ (string)$filiere->id === $filiereId ? 'selected' : '' }}>{{ $filiere->nom }}</option>
                                @endforeach
                            </select>
                            ...
                            <div class="relative">
                                <select name="niveau_id" x-model="niveauId" @change="onNiveauChange()"
                                    ...
                                    <template x-for="n in niveaux" :key="n.id">
                                        <option :value="String(n.id)" :selected="String(n.id) === niveauId" x-text="n.nom"></option>
                                    </template>
                                </select>
                            ...
                            <div class="relative">
                                <select name="groupe_id" x-model="groupeId"
                                    ...
                                    <template x-for="g in groupes" :key="g.id">
                                        <option :value="String(g.id)" :selected="String(g.id) === groupeId" x-text="g.nom"></option>
                                    </template>
                                </select>
                ...
                <div x-show="role === 'professeur'" x-cloak
                    ...
                    <div class="flex items-center gap-2 mb-2">
                        ...
                        @if($user->isProfesseur() && $user->professeur)
                            <span class="ml-auto text-xs text-gray-400">
                                {{ __('Matricule') }}: {{ $user->professeur->matricule }}
                            </span>
                        @endif
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div class="space-y-2">
                            ...
                            <select name="grade"
                                ...
                                @php $currentGrade = old('grade', $user->professeur->grade ?? 'assistant'); @endphp
                                <option value="assistant" {{ $currentGrade == 'assistant' ? 'selected' : '' }}>{{ __('Assistant') }}</option>
                                <option value="maitre_assistant" {{ $currentGrade == 'maitre_assistant' ? 'selected' : '' }}>{{ __('Maître Assistant') }}</option>
                                <option value="professeur" {{ $currentGrade == 'professeur' ? 'selected' : '' }}>{{ __('Professeur') }}</option>
                                <option value="vacataire" {{ $currentGrade == 'vacataire' ? 'selected' : '' }}>{{ __('Vacataire') }}</option>
                            </select>
                ...
                <div class="flex items-center justify-end gap-4 pt-6 border-t border-gray-100 dark:border-gray-700">
                    ...
                    <button type="submit" class="btn-primary px-8">{{ __("Mettre à jour l'utilisateur") }}</button>
                </div>
            </form>
        </div>
    </div>
