        ...
        <table class="w-full text-left border-collapse">
            <thead>
                <tr class="bg-gray-50 dark:bg-gray-900/50 border-b border-gray-100 dark:border-gray-700">
                    <th class="px-6 py-4 text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Étudiant</th>
                    <th class="px-6 py-4 text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Module</th>
                    <th class="px-6 py-4 text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Date & Heure</th>
                    <th class="px-6 py-4 text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider text-center">Statut</th>
                    <th class="px-6 py-4 text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider text-center">Justificatif</th>
                    ...
                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <p class="text-[10px] text-gray-500">{{ $absence->etudiant->groupe->nom ?? 'Sans groupe' }}</p>
                                </div>
                            </div>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-center">
                            @if($absence->justifiee)
                                <span class="px-2.5 py-1 rounded-full text-[10px] font-bold uppercase bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400">Justifiée</span>
                            @else
                                <span class="px-2.5 py-1 rounded-full text-[10px] font-bold uppercase bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400">Non Justifiée</span>
                            @endif
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-center">
                            @if($absence->justificatif)
                                ...
                                <div class="flex flex-col items-center gap-1">
                                    ...
                                    <a href="{{ asset('storage/' . $absence->justificatif->fichier) }}" target="_blank" class="text-[10px] text-indigo-500 hover:underline font-bold">Voir le doc</a>
                                </div>
                            @else
                                <span class="text-gray-300 dark:text-gray-600 text-[10px]">Aucun</span>
                            @endif
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-right">
                            <div class="flex items-center justify-end gap-2">
                                ...
                                <form action="{{ route('admin.absences.toggle-justifiee', $absence) }}" method="POST">
                                    @csrf
                                    @method('PATCH')
                                    <button type="submit" class="p-2 rounded-lg {{ $absence->justifiee ? 'text-red-500 hover:bg-red-100' : 'text-green-500 hover:bg-green-100' }} transition-all" title="Basculer la justification">
                                        @if($absence->justifiee)
                                ...
                                @if($absence->justificatif && $absence->justificatif->statut === 'en_attente')
                                    <div class="flex items-center gap-1 border-l border-gray-100 dark:border-gray-700 pl-2">
                                        <a href="{{ route('admin.justificatifs.valider', $absence->justificatif) }}" class="p-2 rounded-lg text-green-600 hover:bg-green-50 dark:hover:bg-green-900/20" title="Valider le justificatif">
                                        ...
                                        <button type="button" @click="Swal.fire({
                                            ...
                                            inputPlaceholder: 'Ex: Document illisible',
                                        ...
                                        })" class="p-2 rounded-lg text-red-600 hover:bg-red-50 dark:hover:bg-red-900/20" title="Refuser le justificatif">
