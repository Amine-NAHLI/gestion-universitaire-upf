    ...
    <div class="bg-white dark:bg-gray-800 rounded-3xl border border-gray-100 dark:border-gray-700 shadow-xl overflow-hidden">
        <div class="p-8 border-b border-gray-100 dark:border-gray-700 bg-gray-50/50 dark:bg-gray-900/30">
            <h2 class="text-2xl font-extrabold text-gray-800 dark:text-white">
                {{ isset($module) ? 'Modifier : ' . $module->nom : 'Créer un module' }}
            </h2>
        </div>
        ...
        <form action="{{ isset($module) ? route('admin.modules.update', $module) : route('admin.modules.store') }}" 
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="space-y-2">
                    <label for="nom" class="text-sm font-bold text-gray-700 dark:text-gray-300">Nom du module</label>
                ...
                <div class="space-y-2">
                    <label for="code" class="text-sm font-bold text-gray-700 dark:text-gray-300">Code (ex: M11, GI-01...)</label>
                ...
                <div class="space-y-2">
                    <label for="niveau_id" class="text-sm font-bold text-gray-700 dark:text-gray-300">Filière / Niveau</label>
                ...
                <div class="space-y-2">
                    <label for="semestre" class="text-sm font-bold text-gray-700 dark:text-gray-300">Semestre</label>
                    <select name="semestre" id="semestre" required
                            class="w-full px-4 py-3 rounded-xl border border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-800 dark:text-white focus:ring-2 focus:ring-indigo-500">
                        @foreach(range(1, 6) as $s)
                            <option value="{{ $s }}" {{ old('semestre', $module->semestre ?? '') == $s ? 'selected' : '' }}>Semestre {{ $s }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="space-y-2 md:col-span-2 grid grid-cols-2 md:grid-cols-4 gap-4 bg-gray-50 dark:bg-gray-900/50 p-4 rounded-2xl border border-gray-100 dark:border-gray-700">
                    <div class="space-y-1">
                        <label class="text-[10px] font-bold text-gray-500 uppercase">Coefficient</label>
                    ...
                    <div class="space-y-1">
                        <label class="text-[10px] font-bold text-blue-500 uppercase">Cours (H)</label>
            ...
            <div class="space-y-2">
                <label for="description" class="text-sm font-bold text-gray-700 dark:text-gray-300">Description / Objectifs</label>
            ...
            <div class="flex items-center justify-end gap-4 pt-6 border-t border-gray-100 dark:border-gray-700">
                <a href="{{ route('admin.modules.index') }}" class="btn-secondary">Annuler</a>
                <button type="submit" class="btn-primary px-8">
                    {{ isset($module) ? 'Mettre à jour' : 'Enregistrer' }}
                </button>
            </div>
        </form>
    </div>
