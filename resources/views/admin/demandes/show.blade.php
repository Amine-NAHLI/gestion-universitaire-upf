...
@section('title', __('Détails de la Demande'))
@section('page-title', __('Détails de la Demande'))
            ...
            <div class="bg-white dark:bg-gray-800 rounded-3xl border border-gray-100 dark:border-gray-700 shadow-xl overflow-hidden">
                <div class="p-8 border-b border-gray-100 dark:border-gray-700 bg-gray-50/50 dark:bg-gray-900/30">
                    <div class="flex justify-between items-start">
                        <div>
                            ...
                            <p class="text-sm text-gray-500 mt-1">{{ __('Demande déposée le') }} {{ $demande->created_at->format('d F Y à H:i') }}</p>
                        </div>
                        @php
                            $statutColors = [
                ...
                <div class="p-8 space-y-8">
                    <div class="grid grid-cols-2 gap-8">
                        <div>
                            <h4 class="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2">{{ __('Demandeur') }}</h4>
                        ...
                        <div>
                            <h4 class="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2">{{ __('Email Contact') }}</h4>
                    ...
                    @if($demande->donnees_supplementaires)
                        <div class="p-6 rounded-2xl bg-gray-50 dark:bg-gray-900/50 border border-gray-100 dark:border-gray-700">
                            <h4 class="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-4">{{ __('Informations Complémentaires') }}</h4>
                            <div class="space-y-4">
                    ...
                    @if($demande->statut === 'refusee')
                        <div class="p-6 rounded-2xl bg-red-50 dark:bg-red-900/10 border border-red-100 dark:border-red-900/30">
                            <h4 class="text-[10px] font-black text-red-400 uppercase tracking-widest mb-2">{{ __('Motif du refus') }}</h4>
                    ...
                    @if($demande->statut === 'validee' && $demande->fichier_pdf)
                        <div class="flex items-center justify-between p-4 rounded-2xl bg-indigo-50 dark:bg-indigo-900/10 border border-indigo-100 dark:border-indigo-900/30">
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    <p class="text-sm font-bold text-indigo-700 dark:text-indigo-400">{{ __('Document généré') }}</p>
                                    <p class="text-[10px] text-indigo-500">{{ __('Prêt pour téléchargement') }}</p>
                                </div>
                            </div>
                            <a href="{{ route('admin.demandes.pdf', $demande) }}" class="btn-primary py-2 px-4 text-xs">{{ __('Télécharger') }}</a>
                        </div>
                    @endif
                </div>
            </div>
        ...
        <div class="space-y-6">
            @if($demande->statut === 'en_attente')
                <div class="bg-white dark:bg-gray-800 p-6 rounded-3xl border border-gray-100 dark:border-gray-700 shadow-xl space-y-4">
                    <h3 class="text-sm font-black text-gray-800 dark:text-white uppercase tracking-widest">{{ __('Actions Administrateur') }}</h3>
                    ...
                    <button @click="Swal.fire({
                        ...
                        inputPlaceholder: '{{ __('Indiquez la raison du refus...') }}',
            ...
            <div class="bg-indigo-900 rounded-3xl p-6 text-white overflow-hidden relative group">
                <div class="relative z-10">
                    <h4 class="text-xs font-bold text-indigo-300 uppercase mb-2">{{ __("Besoin d'aide ?") }}</h4>
                    <p class="text-sm font-medium opacity-80 mb-4">Vérifiez les données de l'étudiant avant de valider le document officiel.</p>
                    <a href="{{ route('admin.users.index', ['role' => 'etudiant']) }}" class="text-xs font-bold underline hover:text-indigo-200 transition-colors">{{ __('Consulter le dossier') }}</a>
                </div>
