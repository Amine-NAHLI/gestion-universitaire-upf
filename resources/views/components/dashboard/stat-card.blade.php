@props(['title', 'value', 'icon' => 'chart-bar', 'color' => 'indigo', 'trend' => null])
...
<div class="bg-white dark:bg-gray-800 rounded-2xl shadow-md hover:shadow-xl transition-all duration-300 p-6 transform hover:-translate-y-1" data-aos="fade-up">
    <div class="flex items-center justify-between mb-4">
        <div class="p-3 bg-{{ $color }}-100 dark:bg-{{ $color }}-900/30 rounded-xl">
            <svg class="w-6 h-6 text-{{ $color }}-600 dark:text-{{ $color }}-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                {!! $iconSvg !!}
            </svg>
        </div>
        @if($trend !== null)
            <span class="text-sm font-bold {{ $trend >= 0 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400' }}">
                {{ $trend > 0 ? '+' : '' }}{{ $trend }}%
            </span>
        @endif
    </div>
    <h3 class="text-3xl font-black text-gray-900 dark:text-white mb-1 tracking-tight">{{ $value }}</h3>
